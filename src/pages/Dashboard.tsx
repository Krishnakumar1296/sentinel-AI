...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <section className="corp-bg relative overflow-hidden rounded-2xl border border-[#E2E8F0] p-8 shadow-card lg:p-10">
        <div className="relative z-10">
          <h1 className="text-3xl font-bold tracking-tight text-[#123B5D] lg:text-4xl">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            <span className="text-xs font-medium text-[#64748B]">Try:</span>
            {suggestions.map((s) => (
              <button
                key={s}
                onClick={() => navigate(`/search?q=${encodeURIComponent(s)}`)}
                className="rounded-full border border-slate-200 bg-white/70 px-3 py-1 text-xs text-slate-600 transition hover:border-slate-300 hover:bg-white"
              >
                {s}
              </button>
            ))}
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={CheckCircle2} label="Verified Answers" value="94.8%" delta="3.2%" iconClass="bg-green-50 text-green-600" />
        <StatCard icon={Lightbulb} label="Knowledge Gaps" value="23" delta="8 this month" positive={false} iconClass="bg-amber-50 text-amber-500" />
      </section>
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="card p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-[#172033]">Recent AI Searches</h2>
              <p className="text-sm text-[#64748B]">Your latest knowledge queries</p>
            </div>
            <button onClick={() => navigate('/history')} className="flex items-center gap-1.5 text-sm font-medium text-[#2563EB] hover:text-[#1D4ED8]">
              View all
              <ArrowRight className="h-4 w-4" />
            </button>
          </div>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-semibold text-[#172033]">Authorized Search</h2>
              <p className="text-xs font-medium uppercase tracking-wide text-green-600">System secure</p>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3 border-t border-slate-100 pt-4">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-[#64748B]">Access Scope</span>
              <span className="font-semibold text-[#172033]">12 document collections</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-[#64748B]">RBAC Filtering</span>
